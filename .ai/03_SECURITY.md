...
- Android 编码走 FFmpegKit 进程内库，不 `Process.run` 外部 ffmpeg。`android:largeHeap=true` 仅用于医院包转换，不扩大备份或权限。
- rar/7z 解压后仍做路径穿越检查、symlink 拒绝与体积限额。桌面优先用包内 7-Zip；Android 用纯 Dart `koni_archive` 流式解压（不加载 JNI 7-Zip）。提取器查找不依赖 Unix `which`。
