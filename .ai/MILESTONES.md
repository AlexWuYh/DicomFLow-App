...
| M28 | 帧率只保留常用 5 / 10 / 15，手机上一行放下 | `done` |
...
- GitHub 仓库 description 中英对照，用 CT / MRI 片子而不是只写 DICOM；不写个人网站地址
...
**目标**：转换页帧率只留常用档，手机端一行放得下。

**范围内**

- 选项改为 **5 / 10 / 15 fps**（默认仍 10）
- 流畅档 fps 上限改为 10，避免选了默认 10 再切流畅后芯片消失
- 切到流畅时若当前帧率不在允许列表，落到该档最高可用值

**范围外**：自定义输入框、按序列 DICOM 帧间隔自动 fps。
...
**目标**：窄屏预览时多文件切换不挤占画面，也不靠难发现的横滑小卡片。

**范围内**

- 转换结果页与历史详情窄屏：预览占满剩余高度
- 底部「文件 2 / 4」左右切换；点标题或展开打开完整列表弹层
- 单文件不显示切换条；桌面仍用右侧列表

**范围外**：预览手势左右滑切文件（会和缩放/片号冲突）。
...
**目标**：用户不会把「分享」理解成发送全部转换结果。

**范围内**

- 按钮文案改为「分享此文件」
- 附注「只分享正在预览的文件，不是全部结果」
- 「导出到文件夹」tooltip 标明打包全部结果
...
**目标**：结果操作条不再并排放两个都像「把文件存出去」的按钮。

**范围内**

- 窄屏只保留「分享此文件」+「导出到文件夹」
- Android 不显示单独的保存/下载按钮（当前文件走系统分享）
- 桌面仍保留「在文件夹中显示」
